...
namespace Games.TextComparison.Selectable_scripts
{
    public class ClickableText : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetMouseButtonUp(0) && _isActive)
            {
                ...
                if (!_selected.Contains(linkId))
                {
                    ...
                    _soundUnderline.SoundPencilUnderline();
        ...
        private Vector3[] CreateUnderlineCoords(int linkIndex)
        {
            ...
            int endLine = textField.textInfo
                .characterInfo[linkInfo.linkTextfirstCharacterIndex + linkInfo.linkTextLength].lineNumber;
            ...
            for (int i = startLine; i < endLine + 1; i++)
            {
                ...
                int startIndex = textInfo.lineInfo[i].firstCharacterIndex > linkStartIndex
                    ? textInfo.lineInfo[i].firstCharacterIndex
                    : linkStartIndex;
                int endIndex = 0;
                if (linkEndIndex != 0)
                {
                    endIndex = textInfo.lineInfo[i].lastCharacterIndex - 3 < linkEndIndex
                        ? textInfo.lineInfo[i].lastCharacterIndex - 3
                        : linkEndIndex;
