...
namespace NodeEditorFramework
{
    ...
    public abstract class NodeCanvas : ScriptableObject
    {
        ...
        [NonSerialized] public List<ScriptableObject> SOMemoryDump = new List<ScriptableObject>();
        ...
        public bool Validate(bool repair = true)
        {
            ...
            foreach (Node node in nodes)
            {
                ...
                foreach (ConnectionPort port in node.connectionPorts)
                    if (!port.Validate(node, repair) && !repair)
                        return false;
            }
        ...
        public void CheckConnectivity()
        {
            ...
            {
                int counter = 0;
                foreach (var item in node.connectionPorts)
                {
                ...
                if (counter == 0)
                {
                    throw new Exception("A DialogNode is not connected to anything!");
                }
            }
        }
    }
